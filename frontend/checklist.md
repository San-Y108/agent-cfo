...
- [x] **Console Command Center** — `app/console/`（Agent Hub + 5 页导航 + 边缘胶囊分屏面板）
- [~] **Contribution records 展示 / 输入** — `components/console/modules/treasury.tsx` 内 mock 记录驱动付款计划；**缺独立上传 UI**
- [x] **Payment plan 展示** — Treasury 模块内 5-stage 流水线（Records → Risk → Approval → Execution → Audit）
- [x] **Risk check 展示** — Treasury + Policy 模块（budget / whitelist / limit / token / duplicate；Bob blocked 标红）
- [x] **Human approval** — Treasury ActionPanel 含 Approve & Execute 交互
- [x] **CAW execution result（mock）** — Treasury Execution stage + Wallets 模块（Simulated / mock-testnet）
- [x] **tx hash / audit report** — Treasury Audit stage + `components/ui/copyable-hash.tsx`

## B. Mode & 后端集成

- [x] **Mock mode fallback** — 后端 / CAW 不稳定时可独立跑完整 happy path（`lib/demo/demo-data.ts` 静态数据 + `isMockMode()`；`request()` 在 mock 模式直接抛错以防误打后端）
- [~] **Real mode backend integration** — adapter（`lib/api/*`）+ `runDemoFlow()` 已验证；**Console Treasury 尚未切到 real mode**（仍读 `console-mock.ts`，无 loading / error 态）
