...
/* =============================================================================
 * L2 helper — animate a panel's text + mock items in on stage switch
 * ===========================================================================*/

function animatePanelIn(panelEl: HTMLDivElement | null) {
  if (!panelEl) return;

  const eyebrow  = panelEl.querySelector<HTMLElement>(".pipeline-eyebrow");
  const headline = panelEl.querySelector<HTMLElement>(".pipeline-headline");
  const body     = panelEl.querySelector<HTMLElement>(".pipeline-body");
  const items    = panelEl.querySelectorAll<HTMLElement>(".pipeline-mock-item");
  ...
  if (eyebrow) {
    gsap.fromTo(eyebrow,
      { y: 8, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.32, ease });
  }
  if (headline) {
    gsap.fromTo(headline,
      { y: 22, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.5, ease, delay: 0.06 });
  }
  if (body) {
    gsap.fromTo(body,
      { y: 12, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.4, ease, delay: 0.16 });
  }
  if (items.length) {
    gsap.fromTo(items,
      { x: 6, opacity: 0 },
      { x: 0, opacity: 1, duration: 0.32, stagger: 0.055, ease, delay: 0.12 });
  }
}
...
export function PipelineShowcase() {
  ...
  const panelRefs       = useRef<(HTMLDivElement | null)[]>([]);  // L2
  const prevActiveRef   = useRef<number>(-1);                    // L2
  ...
  useGSAP(
    () => {
      ...
      // Kick the first panel in immediately
      animatePanelIn(panelRefs.current[0]);
      ...
      gsap.to(trackRef.current, {
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            // L2 — text reveal on stage change
            if (activeIdx !== prevActiveRef.current) {
              prevActiveRef.current = activeIdx;
              animatePanelIn(panelRefs.current[activeIdx]);
            }
          },
        },
      });
...
function Panel({
  ...
  // Each stage gets a slightly different float speed so they don't sync
  const floatDuration = 6.5 + index * 0.5;
  const floatDelay    = index * 0.9;
  // Ambient glow breathe — slightly offset per stage
  const ambientDuration = 8 + index * 0.7;
  const ambientDelay    = index * 0.6;

  return (
    <div
      ref={panelRef}
      className="relative flex h-screen w-screen flex-shrink-0 items-center justify-center overflow-hidden px-6 lg:px-20"
    >
      {/* L3 — Ambient background glow (breathes) */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0"
        style={{
          background: `radial-gradient(ellipse 55% 45% at 50% 55%, ${stage.accent}, transparent)`,
          animation: `pipeline-ambient ${ambientDuration}s ease-in-out infinite`,
          animationDelay: `${ambientDelay}s`,
          willChange: "opacity, transform",
        }}
      />

      {/* L1 — Giant ghost number (floats vertically) */}
      <span
        aria-hidden
        ...
        style={{
          ...
          right: isEven ? "auto" : "-6%",
          top: "50%",
          animation: `pipeline-float ${floatDuration}s ease-in-out infinite`,
...
/* =============================================================================
 * TEXT BLOCK — headline + body (receives L2 class names)
 * ===========================================================================*/

function TextBlock({ stage }: { stage: Stage }) {
  // L1 — accent dot animation-delay index derived from accent color uniqueness
  const dotAnimDelay = { "#5EEAD4": 0, "#FB7185": 0.8, "#B5FF4D": 1.6, "#60A5FA": 2.4, "#C084FC": 3.2 }[stage.accent] ?? 0;

  return (
    <div>
      {/* L2 target: pipeline-eyebrow */}
      <div
        className="pipeline-eyebrow inline-flex items-center gap-2 rounded-full border px-3 py-1.5"
        style={{ borderColor: stage.accentBorder, backgroundColor: stage.accentSoft }}
      >
        {/* L1 — accent dot breathes */}
        <span
          className="h-1.5 w-1.5 rounded-full"
          style={{
            backgroundColor: stage.accent,
            boxShadow: `0 0 8px ${stage.accent}`,
            animation: `pipeline-dot-breathe 3.2s ease-in-out infinite`,
            animationDelay: `${dotAnimDelay}s`,
      ...
      {/* L2 target: pipeline-headline */}
      <h3
        className="pipeline-headline mt-6 font-medium text-white"
        style={{
          fontFamily: "Inter, sans-serif",
      ...
      {/* L2 target: pipeline-body */}
      <p
        className="pipeline-body mt-6 max-w-md text-sm leading-relaxed text-white md:text-base"
        style={{ fontFamily: "Inter, sans-serif" }}
      >
        {stage.body}
      </p>
    ...
    <div className="grid gap-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {inputSources.map((s) => (
          <div
            key={s.name}
            className="pipeline-mock-item flex items-center gap-2 rounded-[10px] border bg-[#141414] px-2.5 py-2 text-[10px]"
            style={{ fontFamily: "'Courier New', Courier, monospace", borderColor: "rgba(255,255,255,0.15)" }}
          >
      ...
      >
        ...
        >
          ...
          <span className="text-[10px] text-white/65" style={{ fontFamily: "'Courier New', Courier, monospace" }}>
            50 USDC budget
          </span>
        </div>
        {/* L2: plan rows */}
        {planRows.map((r, i) => (
          <div
            key={r.name}
            className={`pipeline-mock-item flex items-center justify-between px-4 py-2.5 text-xs ${
          ...
          >
            ...
            <span className="ml-3 font-semibold" style={{ color: stage.accent }}>{r.amount}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="grid gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="rounded-md border px-2 py-1 text-[10px] font-bold uppercase tracking-[0.15em]"
          style={{ fontFamily: "'Courier New', Courier, monospace", borderColor: stage.accentBorder, backgroundColor: stage.accentSoft, color: stage.accent }}>
          1 blocked
        </div>
      </div>
      ...
      <div className="grid gap-2">
        {gates.map((g) => {
          ...
          return (
            <div
              key={g.name}
              className="pipeline-mock-item flex items-center justify-between rounded-[10px] border px-3.5 py-2.5"
              style={{
            ...
            >
              <div className="flex flex-col">
                <span className="text-xs font-semibold" style={{ color: blocked ? stage.accent : "rgba(255,255,255,0.9)" }}>
                  {g.name}
                </span>
              ...
              <div
                ...
                style={{ backgroundColor: blocked ? stage.accentSoft : "rgba(94,234,212,0.08)", color: blocked ? stage.accent : "#5EEAD4" }}
              >
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div
          className="pipeline-mock-item rounded-[12px] border p-4"
        ...
        >
          ...
          <div className="mt-3 space-y-1.5">
            {approvedQueue.map((q) => (
              <div key={q.name} className="flex items-center justify-between text-[11px]"
                style={{ fontFamily: "'Courier New', Courier, monospace" }}>
        ...
        <div
          className="pipeline-mock-item rounded-[12px] border bg-[#141414] p-4"
        ...
        >
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 text-[10px] text-white/60" style={{ fontFamily: "'Courier New', Courier, monospace" }}>
      ...
      {/* L2: CTA button */}
      <button
        className="pipeline-mock-item group flex items-center justify-between rounded-[12px] px-5 py-4 transition-opacity hover:opacity-90"
        style={{ backgroundColor: stage.accent, color: "#0D0D0D", fontFamily: "Inter, sans-serif" }}
      >
        <span className="text-sm font-bold tracking-tight">Approve & execute · 3 payments</span>
    ...
    >
      ...
      >
        ...
        <span
          ...
          style={{ fontFamily: "'Courier New', Courier, monospace", borderColor: stage.accentBorder, color: stage.accent, backgroundColor: "rgba(96,165,250,0.06)" }}
        >
          testnet-simulated
        </span>
      </div>
      ...
      <div>
        {txs.map((t, i) => (
          <div
            key={t.hash}
            className={`pipeline-mock-item flex items-center justify-between px-5 py-3 ${
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <span
                ...
                style={{ borderColor: stage.accentBorder, color: stage.accent, backgroundColor: stage.accentSoft }}
              >
                {t.amount} ✓
              </span>
            </div>
    ...
    >
      ...
      <div className="grid gap-3 p-5">
        ...
        <div className="rounded-[10px] border border-white/[0.12] bg-[#141414]">
          ...
          <div className="px-3.5 py-2.5">
            ...
            {["0xae3f...2c91", "0x8b21...4ee0", "0x4c7d...91b3"].map((h) => (
              <div
                key={h}
                className="pipeline-mock-item flex items-center justify-between py-1 text-[11px]"
                style={{ fontFamily: "'Courier New', Courier, monospace" }}
              >
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px]" style={{ fontFamily: "'Courier New', Courier, monospace" }}>
          <div className="pipeline-mock-item rounded-md border border-white/[0.12] bg-[#141414] px-3 py-2">
        ...
        <button
          className="pipeline-mock-item mt-1 flex items-center justify-center gap-2 rounded-[10px] border px-4 py-2.5 text-xs font-bold transition-opacity hover:opacity-90"
          style={{ borderColor: stage.accentBorder, backgroundColor: stage.accentSoft, color: stage.accent, fontFamily: "Inter, sans-serif" }}
        >
...
function KPI({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="pipeline-mock-item rounded-[10px] border border-white/[0.12] bg-[#141414] px-3 py-2.5">
      <div
      ...
      <div className="mt-1 text-base font-bold" style={{ fontFamily: "Inter, sans-serif", color }}>
        {value}
      </div>
    </div>
  );
