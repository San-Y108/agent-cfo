  ...
  const TestimonialCard = ({
    ...
    return (
      <motion.div
        ...
        key={String(movedOut)}
        className={cn(
          "absolute z-20 flex items-center gap-2 rounded-md bg-white p-4 opacity-20 shadow-lg dark:bg-neutral-800 lg:opacity-100",
          className
        )}
        animate={{
          y: movedOut ? 300 : 0,
      ...
      >
        ...
        />
        <div>
          <h3 className="text-xs text-neutral-800 dark:text-neutral-200 md:text-base">
            {testimonial.name}
          </h3>
          <p className="max-w-md text-[10px] text-neutral-600 dark:text-neutral-400 md:text-sm">
            {testimonial.quote}
          </p>
        </div>
      </motion.div>
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden">
      ...
      <TestimonialCard
        ...
        direction="right"
      />
      <div className="relative flex h-screen flex-col items-center justify-center overflow-hidden bg-gray-50 px-4 dark:bg-neutral-900 md:px-8">
        <div className="absolute inset-0 z-30 h-full w-full bg-white opacity-80 dark:bg-neutral-900 md:opacity-0" />
        <Image
          src="https://assets.aceternity.com/pro/logos/aceternity-ui.png"
          alt="Aceternity Logo"
          width={100}
          height={100}
          className="aspect-square relative z-50 w-40 object-contain filter dark:invert"
        />
        <h1
          className={cn(
            "relative z-50 mx-auto mt-10 max-w-5xl text-center text-lg font-semibold text-neutral-700 dark:text-neutral-100 sm:text-2xl md:text-4xl lg:text-7xl",
            outfit.className
          )}
        >
