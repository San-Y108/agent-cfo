...
const toneGlow: Record<BentoTone, string> = {
  neutral: "hover:shadow-[0_0_24px_-4px_rgba(255,255,255,0.08)] hover:border-neutral-600",
...
export function BentoCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900/50 p-6 backdrop-blur-sm transition-all duration-300",
        toneGlow[tone],
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      {children}
    </motion.div>
  );
}
