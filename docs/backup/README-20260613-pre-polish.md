...
| `AGENTS.md` | 全 AI agent 工作指南（仓库地图、GitNexus、协作纪律） |
| `CLAUDE.md` | Claude Code 团队边界与阅读顺序 |
| `app/` | FastAPI MVP 服务代码 |
| `tests/` | pytest API 流程测试 |
| `frontend/` | Next.js 前端（Vercel 已部署） |
| `requirements.txt` | Python 依赖锁定版本 |
| `docs/` | 文字文档：PM、P2 边界、阶段报告（见 `docs/README.md`） |
| `assets/` | 交付资产：PPT、答辩视频、截图、海报、Logo（见 `assets/README.md`） |
| `inbox/` | 待归类投递区；整理后迁入 `assets/` 或 `docs/`（见 `inbox/README.md`） |

交付物速查：PPT → `assets/ppt/agentcfo-pitch.pptx` · 物料 PPT（PDF）→ `assets/ppt/material/agentcfo-pitch-material-team-v1.pdf` · 视频 → `assets/video/` · 待归类投递 → `inbox/`（见 `inbox/README.md`）
...
2. `AGENTS.md` 或 `CLAUDE.md`：确认团队边界与目录职责。
3. `app/`：查看 FastAPI 路由、模型、风控服务和 CAW adapter。
4. `tests/`：查看 P0 mock API 的可执行验收用例。
5. `docs/`：任务看板、站会、风险、提交和彩排清单（入口 `docs/README.md`）。
6. `assets/`：路演 PPT、视频、截图与物料（入口 `assets/README.md`）。
7. `inbox/`：队友新投递、待归类文件（入口 `inbox/README.md`）；整理后迁入 `assets/` 或 `docs/`。

前端开发另读 `frontend/CLAUDE.md`；前端专项文档在 `frontend/docs/`。
...
文档入口：[`docs/README.md`](docs/README.md) · 交付资产：[`assets/README.md`](assets/README.md) · 待归类投递：[`inbox/README.md`](inbox/README.md)
...
| 物料 / 设计 / 内容 | 新文件投 `inbox/`；归类后维护 `assets/` 下 PPT、海报、视频、截图 |
| 前端 | 产品界面 + Mock 模式 |
...
- `AGENTS.md` / `CLAUDE.md` 与 README 的仓库结构、部署、交付物路径保持一致。
- `docs/` 放文字文档；`assets/` 放已归类交付物；`inbox/` 放待整理投递；`frontend/docs/` 仅放前端开发文档。
- 代码里的 `app/models.py`、`app/routers/payments.py`、`app/services/` 是当前 P0 API 和业务规则的准确信息来源。
- 需求或交付物路径变化时，同步更新 README、AGENTS.md、CLAUDE.md 及 `docs/pm/SUBMISSION_CHECKLIST.md`。

## Demo Video

> 待录制。视频文件放在 [`assets/video/`](assets/video/)，公开链接写在本节下方。最终提交前必须替换为 3–5 分钟 Demo 视频链接。当前已有 1 笔低额 CAW testnet tx hash；视频需明确说明它只证明 1 笔 testnet transfer，默认 mock fallback 仍保留。

路演 PPT（ppt-master）：[`assets/ppt/agentcfo-pitch.pptx`](assets/ppt/agentcfo-pitch.pptx)（14 页，含演讲备注）

路演 PPT（物料同学 PDF）：[`assets/ppt/material/agentcfo-pitch-material-team-v1.pdf`](assets/ppt/material/agentcfo-pitch-material-team-v1.pdf)
