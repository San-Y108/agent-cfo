...
Backend has completed a demo-safe P2 backend spike. The shipped scope is metadata, preview, and reference only; live P2 integrations are still not approved.
...
| P2 live implementation | Not approved |
...
| --- | --- | --- |
| Request Network invoice records | Demo-safe backend complete; live not ready | Needs Request Finance API key and approval for live API calls |
...
- Request Network docs: API supports programmatic payment destinations, secure payments, payouts, and webhooks; AgentCFO currently stores mock invoice metadata only.
...
- Live Request Finance integration requires a Request Finance API key and explicit approval.
